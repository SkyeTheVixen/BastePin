...
<?php
    ...
    $sql = "SELECT * FROM `tblBastes` WHERE (`tblBastes`.`Visibility` = 2 OR `tblBastes`.`UserID` = ?) AND (`tblBastes`.`ExpiresAt` > NOW() OR `tblBastes`.`ExpiresAt` = NULL) ORDER BY `tblBastes`.`CreatedAt` DESC";
